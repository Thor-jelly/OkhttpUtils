    ...
    fun download(
        ...
        /*Timber.tag("123===").d(
            Thread.currentThread()
                .toString() + "===" + (Thread.currentThread() == Looper.getMainLooper().thread)
        )*/
        ...
        val request: Request = Request.Builder()
            ...
            .build()
        //构建我们的进度监听器
        val listener: OkHttpDownloadProgressListener =
            object : OkHttpDownloadProgressListener {
                override fun progress(current: Long, contentLength: Long) {
                    ...
                    Timber.tag("123===")
                        .d("下载进度：" + 100 * current / contentLength + "%" + current)*/
                    Platform.get().execute {
                        //文件下载占比例80% 保存文件占20%
                        downloadListener?.progress(
                            current,
                            contentLength,
                            (current * 100.0 / (1.2 * contentLength)).toString().formatNumber(2, true)
                        )
                    }
                }
            }
        //创建一个OkHttpClient，并添加网络拦截器
        val client: OkHttpClient = oldOkHttpClient.newBuilder()
            .addInterceptor(ProgressIntercept(listener))
            .build()
        //发送响应
...
//                        })
                            ...
                            override fun inProgress(current: Long, total: Long) {
                                ...
                                Platform.get().execute {
                                    ...
                                    downloadListener?.progress(
                                        current,
                                        total,
                                        (80 + current * 100.0 / total * 0.2).toString()
                                            .formatNumber(2, true)
                                    )
                                }
                            }
                    ...
                    //NetPointEvent.addRequestErrorEvent(request, url, errorMsg)
                    Platform.get().execute {
